...
class DeploymentScriptTests(unittest.TestCase):
    ...
    def test_dry_run_from_another_directory_builds_only_cloud_run_source(self):
        ...
        self.assertIn("GOOGLE_CLOUD_PROJECT=test-project", result.stdout)
        self.assertIn("GOOGLE_CLOUD_LOCATION=global", result.stdout)
        self.assertIn("--clear-secrets", result.stdout)
        self.assertNotIn("--update-secrets", result.stdout)
        self.assertNotIn("secretmanager.googleapis.com", result.stdout)
        self.assertNotIn(" run services delete ", result.stdout)
        self.assertIn("roles/aiplatform.user", result.stdout)
