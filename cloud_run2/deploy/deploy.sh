...
usage() {
    cat <<'EOF'
Cloud Run ADC 소스 배포 (저장소 내 어느 디렉터리에서도 실행 가능)

  PROJECT_ID=my-project bash cloud_run2/deploy/deploy.sh [--dry-run]

필수: PROJECT_ID, 프로젝트에 연결된 결제 계정
선택 환경변수:
  REGION=us-central1                SERVICE_NAME=gemini-chatbot-adc
  SERVICE_ACCOUNT_ID=gemini-chatbot-adc-run
  BUILD_SERVICE_ACCOUNT_ID=gemini-chatbot-adc-build
  GOOGLE_CLOUD_LOCATION=global      # Gemini 호출 위치 (Cloud Run 리전과 별도)
...
SERVICE_NAME="${SERVICE_NAME:-gemini-chatbot-adc}"
SERVICE_ACCOUNT_ID="${SERVICE_ACCOUNT_ID:-gemini-chatbot-adc-run}"
BUILD_SERVICE_ACCOUNT_ID="${BUILD_SERVICE_ACCOUNT_ID:-gemini-chatbot-adc-build}"
...
run gcloud services enable run.googleapis.com cloudbuild.googleapis.com \
    artifactregistry.googleapis.com aiplatform.googleapis.com iam.googleapis.com \
...
run gcloud projects add-iam-policy-binding "$PROJECT_ID" \
    --member="serviceAccount:${RUNTIME_SA}" --role=roles/aiplatform.user --condition=None --quiet --format=none
run gcloud projects add-iam-policy-binding "$PROJECT_ID" \
    --member="serviceAccount:${BUILD_SA}" --role=roles/run.builder --condition=None --quiet --format=none
...
run gcloud run deploy "$SERVICE_NAME" \
    ...
    --clear-secrets \
    --remove-env-vars=GEMINI_API_KEY,GOOGLE_API_KEY,GOOGLE_APPLICATION_CREDENTIALS \
    --update-env-vars="GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=${GOOGLE_CLOUD_LOCATION},CHAT_TIMEOUT_SECONDS=${CHAT_TIMEOUT_SECONDS}" \
